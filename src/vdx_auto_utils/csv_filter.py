...
class CSVFilter:
    ...
    def __init__(self, input_source):
        ...
        if isinstance(input_source, pd.DataFrame):
            ...
            raise TypeError(
                "Input must be either a file path (string) or a Pandas DataFrame."
            )
    ...
    def _drop_columns(self, columns_to_remove: List[str]):
        ...
        if invalid_cols:
            print(
                f"-> Warning: Columns not found and skipped during removal: {invalid_cols}"
            )
    ...
    def _apply_filtering_criteria(self, filter_criteria: Dict[str, Any]):
        ...
        for column, criterion in filter_criteria.items():
            if column not in self.df.columns:
                print(
                    f"-> Warning: Filter column '{column}' not found. Skipping filter."
                )
                continue
            ...
            for single_criterion in criteria_list:
                ...
                ) and single_criterion.upper().startswith("CONTAINS:"):
                ...
                # Handling EXACT operator
                    ...
                    single_criterion, str
                ) and single_criterion.upper().startswith("EXACT:"):
                ...
                # Handling comparison operators (>, <, etc.)
                elif isinstance(single_criterion, str) and any(
                    op in single_criterion for op in [">", "<", ">=", "<="]
                ):
                    op = (
                        single_criterion[0:2]
                        if single_criterion[1] == "="
                        else single_criterion[0]
                    )
                    ...
                    if op == ">":
                        ...
                        print(
                            f"-> Warning: Unsupported operator '{op}' for column '{column}'. Skipping criterion."
                        )
    ...
    def _apply_multiplier_math(self, math_rules: List[Dict[str, Any]]):
        ...
        for rule in math_rules:
            ...
            if target not in self.df.columns or source not in self.df.columns:
                print(
                    f"Warning: Columns {target} or {source} not found. Skipping math rule."
                )
                continue
            ...
            if rows_affected > 0:
                ...
                print(
                    f"   -> Updated {rows_affected} rows in '{target}' using '{source}' * {multiplier}"
                )
    ...
    def _split_column(self, split_rules: Union[Dict[str, Any], List[Dict[str, Any]]]):
        ...
        try:
            ...
            split_data = (
                self.df[target].astype(str).str.split(delimiter, n=1, expand=True)
            )
            ...
            if len(split_data.columns) > 1:
                ...
                self.df.loc[:, new_headers[1]] = (
                    ""  # Fill with empty string if no split occurred
                )
    ...
    def apply_filters(
        self, filter_rules: Dict[str, Any], execution_order: List[str]
    ) -> pd.DataFrame:
        """
        ...
        for step_name in execution_order:
            if step_name not in OPERATION_MAP:
                print(
                    f"Warning: Unknown operation '{step_name}' requested in execution_order. Skipping."
                )
                continue
            ...
            if (
                params is not None
                and (isinstance(params, (list, dict)) and len(params) > 0)
                or step_name == "handle_na"
            ):
                try:
                    method(params)
                except Exception as e:
                    ...
                    print(
                        f"🛑 Error during step '{step_name}' using parameter '{param_key}': {e}"
                    )
                    traceback.print_exc()
                    raise
            else:
